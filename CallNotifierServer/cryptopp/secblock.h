...
typename A::pointer StandardReallocate(A& a, T *p, typename A::size_type oldSize, typename A::size_type newSize, bool preserve)
{
	...
	if (preserve)
	{
		...
		memcpy_s2(newPointer, sizeof(T)*newSize, p, sizeof(T)*STDMIN(oldSize, newSize));
...
public:
	...
	SecBlock(const SecBlock<T, A> &t)
		: m_size(t.m_size) {m_ptr = m_alloc.allocate(m_size, NULL); memcpy_s2(m_ptr, m_size*sizeof(T), t.m_ptr, m_size*sizeof(T));}
...
//		{assert(index >= 0 && index < m_size); return m_ptr[index];}
	...
	{
		New(len);
		memcpy_s2(m_ptr, m_size*sizeof(T), t, len*sizeof(T));
	}
	...
	void Assign(const SecBlock<T, A> &t)
	{
		...
		{
			New(t.m_size);
			memcpy_s2(m_ptr, m_size*sizeof(T), t.m_ptr, m_size*sizeof(T));
		}
	}
	...
	SecBlock<T, A>& operator+=(const SecBlock<T, A> &t)
	{
		...
		memcpy_s2(m_ptr+oldSize, m_size*sizeof(T), t.m_ptr, t.m_size*sizeof(T));
		return *this;
	}
	...
	SecBlock<T, A> operator+(const SecBlock<T, A> &t)
	{
		...
		memcpy_s2(result.m_ptr, result.m_size*sizeof(T), m_ptr, m_size*sizeof(T));
		memcpy_s2(result.m_ptr+m_size, t.m_size*sizeof(T), t.m_ptr, t.m_size*sizeof(T));
		return result;
	}
